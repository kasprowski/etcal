 ...
 */
public class Test5 {
	static String fdir = "c:/etcal_data/et/";
	static String reportFile = "c:/etcal_data/test5";

	public static void main(String[] args) throws Exception{
		for(String fname:new File(fdir).list()) 
			if(fname.contains("train"))
				new Test5().doExperiment(fname.substring(0,fname.indexOf("_")));
	...
	public void doExperiment(String fname) throws Exception{
		...
		for(int vars:new int[] {2,6,10}) {
			duTrain = DataUnits.load(fdir+"/"+fname+"_train.json");
			duTest = DataUnits.load(fdir+"/"+fname+"_test.json");
			duTrain = TestHelper.cutVariables(duTrain, vars);
			duTest = TestHelper.cutVariables(duTrain, vars);
			...
			printResults("quad"+vars,calibpar);

			mask = TestHelper.genMask(duTrain.xNum(), 1);
			calibpar.params.put("mask",mask);
			etcal.build(calibpar);
			printResults("linear"+vars,calibpar);
		}
	}
